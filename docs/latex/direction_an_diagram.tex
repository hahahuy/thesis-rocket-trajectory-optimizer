...
\begin{document}
\begin{tikzpicture}[
    node distance=0.8cm and 1.75cm,
...
\node[process, below=0.8cm of A] (B) {TimeEmbedding\\FourierFeatures\\8 frequencies};
\node[process, below=0.8cm of C] (D) {ContextEncoder\\7→128, Tanh};
...
% Mission branches
\node[branch, below left=0.8cm and 1.5cm of J] (K1) {TranslationBranch\\128→128→128→6};
\node[branch, below=of J] (K2) {RotationBranch\\128→256→256→7};
\node[branch, below right=0.8cm and 1.5cm of J] (K3) {MassBranch\\128→64→1};
...
% Physics layer
\node[physics, below left=2cm and 2cm of O] (P) {Physics Residual Layer\\Autograd-based\\compute\_dynamics};
\node[process, below=of P] (R) {PhysicsResiduals\\ODE residuals};

% Final output
\node[output, below right=1cm and 2cm of O] (S) {Output\\(state, residuals)};
...
\draw[arrow] (A) -- ++(-5.25cm,-1.5cm) |- ([yshift=0.3cm]P.west);
