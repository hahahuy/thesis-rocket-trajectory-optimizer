...
PINN-inspired surrogates were trained on these datasets
to approximate optimal ascent trajectories implied by the dynamics. The relationship between the optimal control solver,
...
method based on the Hermite–Simpson scheme~\cite{betts2010,ross2004}. Which served 
...
aerodynamic coefficients, and mass dynamics, but used tensor operations to enable formal 
automatic differentiation of the surrogate.
...
\(T_\mathrm{max}\), and normalised state targets. Although the context vector 
includes a wind magnitude parameter for extensibility, wind disturbances were 
set to zero in all experiments reported in this thesis.
...
Unlike classical PINNs that enforce differential equations via automatic differentiation 
at collocation points, the present approach employs finite-difference approximations of 
temporal derivatives on discretized trajectories, and is therefore described as PINN-inspired 
rather than a strict PINN formulation.
...
(\(\mathbf{u}_k = [0, 0, 0, 0]^\top\)).It is emphasized that this residual does 
not correspond to the true flight dynamics; instead, it acts as a structural 
regularizer that biases the learned trajectories toward smooth, physically plausible 
manifolds consistent with the reduced, uncontrolled dynamics. It is important 
to note that this zero-control assumption was consistent with the processed dataset 
format, which did not include control trajectories.

As a result, the physics residual does not represent the full controlled dynamics, but instead acts as a regularizer that biases the learned trajectories toward consistency with the uncontrolled structure of the governing equations. Because control inputs are omitted, the residual does not represent the true flight dynamics, but instead biases the learned trajectories toward a physically structured manifold.

Accordingly, the residual should be interpreted as a physics-based regularizer rather than an enforcement of the full controlled equations of motion.
...
terms enforcing data fidelity, physics-based regularization, and structural constraints. 
...
In addition to the core data and physics losses, several auxiliary regularization terms 
were introduced as inductive biases to stabilize training and encode known symmetries of 
the ascent problem; these terms are not intended to represent independent physical laws. 
...
\begin{enumerate}
  ...
  \item Physics-based regularization losses, which penalized violations of a reduced form of the governing equations 
...
These auxiliary loss terms should be interpreted as inductive biases encoding known symmetries and qualitative properties of the ascent problem, rather than as substitutes for the governing equations themselves. Several constraints intentionally overlap in their effect to stabilize learning in symmetry-dominated dimensions and do not represent independent physical laws.
...
representation used during training, and RMSE does not capture localized constraint 
violations, which are examined separately in Section 3.4
...
fidelity, physics-based regularization, structural constraints, and smoothing
